...
export const makeDshAdapter = Effect.fn("makeDshAdapter")(function* (
  ...
  const handleMuxFrame = (envelope: RpcRequest<MuxFrame>): void => {
    ...
    // Use crypto.randomUUID for uniqueness; EventId is branded string
    const eventId = EventId.make(`dsh-mux-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
      ...
      switch (frame.type) {
        case "session/event": {
          ...
          evt = {
            ...
            threadId: tid,
            type: "item.updated",
            payload: { data: f.event, view: (f as unknown as { view?: unknown }).view },
        ...
        case "session/subscribed": {
          ...
          // Also seed session status tracking if unknown
          if (!sessions.has(tid)) {
            // Ensure session is known for later mapping; keep minimal context
            // Do not create full context, just allow threadId resolution via fallback above
          }
          break;
        }
        case "approval/requested": {
          ...
          evt = {
            ...
            payload: {
              ...
              // Stash raw identifiers in payload for downstream approval UI
              args: {
        ...
        case "question/requested": {
          ...
          const tid = threadIdForSessionId(f.sessionId);
          // Map AskUserQuestionItem[] directly; harness questions have id/header/question/options
        ...
        case "stream/error": {
          ...
          evt = {
            ...
            type: "runtime.error",
            payload: { message: f.error.message ?? String(f.error) },
          } as unknown as import("@t3tools/contracts").ProviderRuntimeEvent;
          break;
        }
        default: {
